...
if(movement.getPCDebugging()){
		...
		isPressingThis = false;
   		...
   		if(isPressingThis && (Input.GetMouseButtonUp(0)&& GetComponent.<GUITexture>().HitTest(Input.mousePosition)) && PlayerPrefs.GetInt("CheckPointRigUnlocked",0) == 1){
   			movement.setRig("CheckPointRig");
    ...
    if(movement.getIsInGameMode() &&    PlayerPrefs.GetString("Rig") == "CheckPointRig"){
    	GetComponent.<GUITexture>().color = Color.green;
    	GetComponent.<GUITexture>().color.a = 0.35;
    }else if(PlayerPrefs.GetInt("CheckPointRigUnlocked",0) == 1){
