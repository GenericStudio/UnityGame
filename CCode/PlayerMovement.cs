...
public class PlayerMovement : MonoBehaviour
{
    ...
    public void handleJumps()
    {
        ...
        if (jump_triggered)
        {
            ...
            if (can_jump)
            {
                ...
                this.currSpeed.y = this.currSpeed.y + (this.staticFrame.y + jump_force);
            }
            else if (this.isOnLeftWall || this.isOnRightWall)
            {
                audio.PlayOneShot(JumpSound);

                this.currSpeed.y = this.staticFrame.y + jump_force / 1.5f;
                this.currSpeed.x = this.currSpeed.x + ((!this.isOnLeftWall) ? ((float)0) : jump_force);
                this.currSpeed.x = this.currSpeed.x - ((!this.isOnRightWall) ? ((float)0) : jump_force);
                this.isStillJumping = true;
            }
        ...
        if (this.Inputs.jump.state == key_state.Up)
        {
            ...
            {
                this.currSpeed.y = this.currSpeed.y - Mathf.Min(this.currSpeed.y / (float)2, jump_force / (float)2);
            }
            this.isStillJumping = false;
        }
        bool A;
        if (A = !this.isTouchingFloor)
        {
            A = !this.isStillJumping;
        }
        bool B;
        if (B = A)
        {
            B = (this.Inputs.jump.state == key_state.Hold);
        }
    ...
    public void handleHorizontal()
    {
        ...
        this.speedMax = (this.Inputs.down.state == key_state.None || !isTouchingFloor) ? this.runSpeed : this.runSpeed / 3 + Mathf.Abs(this.staticFrame.x);
        float difference = this.speedMax - (float)ground_speed_magnitude;
        if (Mathf.Abs(this.horizontalInput) > 0.1f)
        {
            if (Mathf.Sign(this.horizontalInput) == Mathf.Sign((float)this.ground_speed))
            {
                if (difference < 0) difference *= 2;
                this.currSpeed.x = this.currSpeed.x + difference * (Time.fixedDeltaTime * this.accel) * Mathf.Sign(this.horizontalInput);
            }
            else
            {
        ...
        else if (ground_speed_magnitude > 4)
        {
            ...
            {
                this.currSpeed.x = this.currSpeed.x - (float)this.ground_speed * Time.fixedDeltaTime * this.accel;
            }
        }
        else
        {
    ...
    public void KillPlayer()
    {
        ...
        List<Equipment> AttachedEquipment = new List<Equipment>();
        List<Item> AttachedItem = new List<Item>();

        //Detach equipment and hand slots
        AttachedItem.AddRange(transform.Find("Hand").GetComponentsInChildren<Item>());
        ...
        AttachedEquipment.AddRange(transform.Find("Feet").GetComponentsInChildren<Equipment>());
        AttachedEquipment.AddRange(transform.Find("Hand").GetComponentsInChildren<Equipment>());
        AttachedEquipment.AddRange(transform.Find("Body").GetComponentsInChildren<Equipment>());
        AttachedEquipment.AddRange(transform.Find("Head").GetComponentsInChildren<Equipment>());
        AttachedEquipment.AddRange(transform.Find("Back").GetComponentsInChildren<Equipment>());

        foreach (var equipment in AttachedEquipment)
        {
            equipment.UnequipToHand(equipment);
            equipment.Item.Drop();
        }
